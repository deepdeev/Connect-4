...
describe('Players Footer', () => {
  if (Meteor.isServer) return;
  it('should render the players score footer with the usernames', () => {
    const game = {
      _id: Random.id(),
      p1: { _id: Random.id(), username: 'username' }, // datos jugador 1
      p2: { _id: Random.id(), username: 'user2' }, // datos jugador 2
    ...
    const item = render(<Game game={game} />);
    assert.equal(item.find('h3').length, 5);
    assert.include(item.find('h3').text(), 'username');
    assert.include(item.find('h3').text(), 'user2');
  });
});
